...
class ActiveDirectory3Controller extends Controller
{
    protected string $connection = 'tertiary';
    protected string $dominio    = 'Unifrutti';
    ...
    public function index(Request $request)
    {
        try {
            ...
            return view('admin.active_directory3.index', compact(
                'usuarios', 'buscar', 'filtroEstado', 'total',
                'countHabilitados', 'countDeshabilitados', 'countTodos'
            ));

        } catch (\Throwable $e) {
            return view('admin.active_directory3.index', [
                'usuarios'            => null,
    ...
    public function edit(string $username)
    {
        try {
            ...
            $returnUrl = url()->previous(route('admin.active_directory3.index'));
            ...
            return view('admin.active_directory3.edit', compact('usuario', 'returnUrl', 'estado'));
        } catch (\LdapRecord\Models\ModelNotFoundException) {
            return redirect()->route('admin.active_directory3.index')
    ...
    public function update(Request $request, string $username)
    {
        ...
        try {
            ...
            $returnUrl = $request->input('_return', route('admin.active_directory3.index'));
    ...
    public function desbloquear(string $username)
    {
        try {
            ...
            return back()->with('success', "Cuenta {$username} desbloqueada en {$this->dominio}.");
    ...
    {
        return view('admin.active_directory3.importar_correos');
    }
    ...
    public function procesarImportacion(Request $request)
    {
        ...
        try {
            ...
            return view('admin.active_directory3.importar_correos', compact('resultados', 'resumen'));
    ...
    private function mensajeError(\Throwable $e): string
    {
        ...
        if (str_contains($msg, 'ldap_bind') || str_contains($msg, 'No credentials')) {
            return 'LDAP (' . $this->dominio . ') no configurado. Completa los datos en Admin → Configuración → AD Unifrutti.';
        }
        if (str_contains($msg, 'No connections exist')) {
            return 'Conexión terciaria no configurada. Completa los datos en Admin → Configuración → AD Unifrutti.';
        }
