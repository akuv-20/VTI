...
<div class="container-fluid vti-page">
    ...
    <div class="vti-page-header">
        <h4><i class="bi bi-diagram-3 me-2"></i>Active Directory — Unifrutti
            @if(isset($total) && $total > 0)
                <span class="badge bg-secondary fw-normal ms-1" style="font-size:.7rem">{{ $total }}</span>
            @endif
        </h4>
        <a href="{{ route('admin.active_directory3.importar_correos') }}" class="btn btn-outline-primary btn-sm">
    ...
    @if(isset($ldapError))
        <div class="alert alert-danger d-flex align-items-start gap-2">
            ...
            <div>
                <strong>No se pudo conectar al AD (Unifrutti):</strong> {{ $ldapError }}<br>
                <a href="{{ route('admin.configuracion.index') }}#pane-ldap3" class="alert-link">
                    Ir a Configuración → AD Unifrutti
                </a>
            </div>
        </div>
    @else
        ...
        {{-- Filtros --}}
        <form method="GET" action="{{ route('admin.active_directory3.index') }}" class="mb-3">
                ...
                <div class="col">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" name="buscar" class="form-control"
                               ...
                               autofocus>
                        @if($buscar)
                            <a href="{{ route('admin.active_directory3.index') }}" class="btn btn-outline-secondary">
                                <i class="bi bi-x"></i>
                            </a>
                        @endif
                    </div>
            ...
            <div class="d-flex gap-2 flex-wrap align-items-center">
                <div class="btn-group btn-group-sm" role="group">
                    <input type="radio" class="btn-check" name="estado" id="estado2_habilitados"
                           value="habilitados" autocomplete="off"
                           {{ $filtroEstado === 'habilitados' ? 'checked' : '' }} onchange="this.form.submit()">
                    <label class="btn btn-outline-success fw-semibold" for="estado2_habilitados">
                        Habilitados <span class="badge bg-success ms-1">{{ $countHabilitados }}</span>
                    </label>

                    <input type="radio" class="btn-check" name="estado" id="estado2_deshabilitados"
                           value="deshabilitados" autocomplete="off"
                           {{ $filtroEstado === 'deshabilitados' ? 'checked' : '' }} onchange="this.form.submit()">
                    <label class="btn btn-outline-danger fw-semibold" for="estado2_deshabilitados">
                        Deshabilitados <span class="badge bg-danger ms-1">{{ $countDeshabilitados }}</span>
                    </label>

                    <input type="radio" class="btn-check" name="estado" id="estado2_todos"
                           value="todos" autocomplete="off"
                           {{ $filtroEstado === 'todos' ? 'checked' : '' }} onchange="this.form.submit()">
                    <label class="btn btn-outline-secondary fw-semibold" for="estado2_todos">
                ...
                <table class="table table-hover align-middle mb-0" style="font-size:.875rem">
                    ...
                    <tbody>
                        ...
                        @php
                            ...
                            $ouSegmentos = array_reverse($ouMatches[1]);
                            $ouPath      = implode(' / ', $ouSegmentos);
                        @endphp
                        <tr class="ad2-fila-click" data-href="{{ route('admin.active_directory3.edit', $sam) }}"
                            ...
                            <td><div class="fw-semibold">{{ $nombre }}</div></td>
                            ...
                            <td>
                                @if($ouSegmentos)
                                    <span class="text-muted" style="font-size:.78rem" title="{{ $ouPath }}">
                            ...
                            <td class="text-end">
                                <div class="d-flex gap-1 justify-content-end">
                                    <a href="{{ route('admin.active_directory3.edit', $sam) }}"
                                       class="btn btn-outline-primary btn-sm" title="Ver estado y editar">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <form action="{{ route('admin.active_directory3.toggle', $sam) }}" method="POST"
                                          data-confirm="{{ $nombre }}"
...
@push('scripts')
<script>
document.querySelectorAll('tr.ad2-fila-click').forEach(function (fila) {
