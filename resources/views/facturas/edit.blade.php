...
<div class="container-fluid vti-page">
    ...
    <div class="vti-page-header">
        <h4>Editar Factura <span class="text-muted fw-normal">— {{ $factura->factura }}</span></h4>
        <a href="{{ route('facturas.index') }}" class="btn btn-outline-secondary btn-sm">
            <i class="bi bi-arrow-left"></i> Volver
        </a>
    </div>

    @php $tipoActual = old('tipo', $factura->tipo); @endphp
    ...
    <form action="{{ route('facturas.update', $factura->id) }}" method="POST">
        @csrf @method('PUT')
        <input type="hidden" name="tipo" id="hiddenTipo" value="{{ $tipoActual }}">
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4">
                    ...
                    <div class="d-flex gap-3">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="_tipo" id="tipoMensual"
                                   value="Mensual" {{ $tipoActual === 'Mensual' ? 'checked' : '' }}>
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="_tipo" id="tipoEsporadica"
                                   value="Esporádica" {{ $tipoActual === 'Esporádica' ? 'checked' : '' }}>
                ...
                {{-- 2a. Mensual: Servicio ─────────────────────────────────────── --}}
                <div id="seccionMensual" class="mb-3 {{ $tipoActual === 'Esporádica' ? 'd-none' : '' }}">
                    ...
                    <select name="id_servicio" id="id_servicio"
                        ...
                        @foreach($servicios as $s)
                            <option value="{{ $s->id }}"
                                    {{ old('id_servicio', $factura->id_servicio) == $s->id ? 'selected' : '' }}>
                ...
                <div id="seccionEsporadica" class="mb-3 {{ $tipoActual !== 'Esporádica' ? 'd-none' : '' }}">
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select name="proveedor" id="sel_proveedor"
                                ...
                                @foreach($companias as $c)
                                    <option value="{{ $c->nombre }}"
                                            {{ old('proveedor', $factura->proveedor) === $c->nombre ? 'selected' : '' }}>
                                        {{ $c->nombre }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="id_cuenta_contable" id="id_cuenta_contable"
                                ...
                                @foreach($cuentasContables as $cc)
                                    <option value="{{ $cc->id }}"
                                            {{ old('id_cuenta_contable', $factura->id_cuenta_contable) == $cc->id ? 'selected' : '' }}>
                ...
                <div class="row g-3 mb-3">
                    <div class="col-md-5">
                        ...
                        <input type="text" name="factura"
                               ...
                               value="{{ old('factura', $factura->factura) }}" placeholder="Ej: 123456">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="oc" class="form-control"
                               value="{{ old('oc', $factura->oc) }}" placeholder="Opcional">
                    </div>
                    <div class="col-md-3">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="descripcion" class="form-control" rows="2"
                              placeholder="Observación o detalle adicional">{{ old('descripcion', $factura->descripcion) }}</textarea>
                </div>
...
(function () {
    ...
    hiddenTipo.value = document.querySelector('input[name="_tipo"]:checked').value;
    ...
    function navegarAlSiguiente() {
        ...
        if (siguiente) { enfocarCampo(siguiente); }
        else { document.querySelector('[type="submit"]').focus(); }
    }
