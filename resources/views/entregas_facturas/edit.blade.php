...
<div class="container-fluid vti-page">
    ...
    <div class="vti-page-header">
        <h4><i class="bi bi-pencil-square me-2"></i>Editar Entrega #{{ $entrega->id }}</h4>
        <a href="{{ route('entregas_facturas.show', $entrega) }}" class="btn btn-outline-secondary btn-sm">
    ...
    <div class="row g-4" style="max-width:1100px">
        ...
        <div class="col-12">
            <form action="{{ route('entregas_facturas.update', $entrega) }}" method="POST" id="form-entrega">
                @csrf
                @method('PUT')
                ...
                <div class="card border-0 shadow-sm rounded-3 mb-4">
                    ...
                    <div class="card-body">
                        <textarea name="observacion" class="form-control form-control-sm"
                                  ...
                                  >{{ old('observacion', $entrega->observacion) }}</textarea>
                    </div>
                </div>
                ...
                <div class="card border-0 shadow-sm rounded-3 mb-4">
                    <div class="card-header fw-bold border-0" style="background:#f8fafc;font-size:.88rem">
                        ...
                        <small class="text-muted fw-normal ms-2">Facturas sin entrega o ya incluidas en ésta</small>
                    </div>
                    <div class="card-body">
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('entregas_facturas.show', $entrega) }}" class="btn btn-outline-secondary">
                        Cancelar
                    </a>
                </div>
...
(function () {
    ...
    // Pre-cargar las facturas ya incluidas en la entrega
    let seleccionadas = {};
    @foreach($seleccionadas as $f)
        seleccionadas[{{ $f['id'] }}] = @json($f);
    @endforeach
    ...
    window.agregarFactura = function (data) {
        ...
        renderSeleccionadas();
        renderResultados(window._ultimoResultado || []);
    };
    ...
    async function buscar(q) {
        ...
        resultadoDiv.innerHTML = '';
        try {
            const url = `{{ route('entregas_facturas.buscar') }}?q=${encodeURIComponent(q)}&entrega=${idEntrega}`;
