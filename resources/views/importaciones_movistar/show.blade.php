...
<div class="container-fluid vti-page">
    ...
    <div class="vti-page-header">
        <div>
            <h4>
                Importación Movistar — Folio <strong>{{ $importacion->folio }}</strong>
            ...
            <small class="text-muted">
                Período: {{ $importacion->periodo_cobro }}
                @if($importacion->fecha_emision)
                    &nbsp;|&nbsp; Emisión: {{ \Carbon\Carbon::parse($importacion->fecha_emision)->format('d/m/Y') }}
                @endif
            </small>
        </div>
        <div class="d-flex gap-2">
            <form action="{{ route('importaciones_movistar.recruzar', $importacion) }}" method="POST"
            ...
            <a href="{{ route('importaciones_movistar.index') }}" class="btn btn-secondary btn-sm">
    ...
    <div class="d-flex gap-3 flex-wrap mb-4">
        ...
        <div class="card border-0 shadow-sm text-center px-4 py-3" style="min-width:130px;background:#f0fdf4">
            <div class="fs-3 fw-bold text-success">{{ number_format($totalEncontradas) }}</div>
        ...
        <div class="card border-0 shadow-sm text-center px-4 py-3" style="min-width:130px;background:#fef2f2">
            <div class="fs-3 fw-bold text-danger">{{ number_format($totalNoEncontradas) }}</div>
    ...
    <div class="d-flex align-items-center gap-2 mb-2">
        ...
        <span class="badge" style="background:#f97316!important">{{ $lineasInactivasFacturadas->count() }}</span>
    </div>
    ...
    {{-- ② Líneas del sistema no en importación --}}
        ...
        <table class="vti-table">
            ...
            <tbody>
                ...
                <tr style="background:{{ $detalle->id_linea_telefonica ? 'rgba(34,197,94,.06)' : 'rgba(239,68,68,.07)' }} !important">
                    ...
                    <td><small class="text-muted">{{ $detalle->producto ?? '—' }}</small></td>
                    <td class="text-end fw-semibold">
                        {{ $detalle->monto !== null ? '$ ' . number_format($detalle->monto, 0, ',', '.') : '—' }}
                    </td>
                    <td>
