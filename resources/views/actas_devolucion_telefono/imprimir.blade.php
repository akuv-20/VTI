...
<head>
    ...
    <title>Acta de Devolución — {{ $acta->numero_telefono }}</title>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .importante strong { color: #92400e; }
...
{{-- Toolbar --}}
<div class="toolbar">
    <a href="{{ route('lineas_telefonicas.show', $acta->id_linea_telefonica) }}">
        <i class="bi bi-arrow-left"></i> Volver
    </a>
    <a href="{{ route('actas_devolucion_telefono.index') }}">
...
<div class="page-wrap">
    ...
    <div class="acta">
        ...
        {{-- Datos generales + equipo en grid --}}
        <div class="datos-grid">
            ...
            <div class="dato-cell">
                <div class="dato-label">Fecha de Devolución</div>
            ...
            <div class="dato-cell">
                <div class="dato-label">N° Teléfono</div>
                <div class="dato-value">{{ $acta->numero_telefono }}</div>
            </div>
            @if($acta->imei_equipo)
            <div class="dato-cell">
                <div class="dato-label">IMEI Equipo</div>
                <div class="dato-value mono">{{ $acta->imei_equipo }}</div>
            </div>
            @endif
            @if($acta->imei_sim)
            <div class="dato-cell">
                <div class="dato-label">IMEI SIM</div>
                <div class="dato-value mono">{{ $acta->imei_sim }}</div>
            </div>
            @endif
        </div>
        ...
        <div class="condicion-row">
            <div class="condicion-item">
                <div class="check-square {{ $acta->condicion === 'Nuevo' ? 'marked' : '' }}">
                    {{ $acta->condicion === 'Nuevo' ? '✓' : '' }}
                </div>
                <span>Nuevo</span>
            </div>
            <div class="condicion-item">
                <div class="check-square {{ $acta->condicion === 'Usado' ? 'marked' : '' }}">
                    {{ $acta->condicion === 'Usado' ? '✓' : '' }}
                </div>
                <span>Usado</span>
            </div>
        </div>
        ...
        {{-- Accesorios --}}
        @php $acc = $acta->accesorios ?? []; $doc = $acta->documentacion ?? []; @endphp

        <div class="section-title">Accesorios Devueltos</div>
        ...
        <table class="check-table">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $label }}</td>
                    <td><span class="check-dot {{ ($doc[$key] ?? '') === 'SI' ? 'si' : '' }}">{{ ($doc[$key] ?? '') === 'SI' ? '✓' : '' }}</span></td>
                    <td><span class="check-dot {{ ($doc[$key] ?? '') === 'NO' ? 'no' : '' }}">{{ ($doc[$key] ?? '') === 'NO' ? '✗' : '' }}</span></td>
                </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div class="firmas">
            <div class="firma-bloque">
                ...
                <div class="firma-rol">Devuelve Conforme</div>
            </div>
            <div class="firma-bloque">
                <div class="firma-linea"></div>
                <div class="firma-nombre">{{ $acta->impreso_por }}</div>
